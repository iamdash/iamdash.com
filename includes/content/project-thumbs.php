<div id="project-images">
    <ul>
        <li class="alpha">
            <a href="/projects/ltm-mtm" title="LTM Exhibiton 'Mind the Map'">
            <img src="/images/projects/ltm-mtm-thumb.png"  alt="" class="cms php-framework exhibition overlay" rel="/projects/ltm-mtm" />
            </a>
        </li>        
        <li>
            <a href="/projects/doolittles" title="Doolittles Dispensary" >
                <img src="/images/projects/doolittles-thumb.png"  alt="doolittlesdispensary.com" class="frontend cms php-framework ecommerce overlay" rel="/projects/doolittles" />
            </a>
        </li>
        <li class="omega">
            <a href="/projects/visa-esta" title="ESTA VISA" >
            <img src="/images/projects/visaesta-thumb.png"  alt="visa-esta-usa.com" class="frontend cms php-framework ecommerce overlay" rel="/projects/visa-esta"  />
            </a>
        </li>
        <li class="alpha">
            <a href="/projects/tesco-opticians" title="Tesco Opticians">
            <img src="/images/projects/tesco-thumb.png"  alt="doolittlesdispensary.com" class="frontend overlay" rel="/projects/tesco-opticians"  />
            </a>
        </li>  
        <li>
            <a href="/projects/nrm" title="National Railway Museum">
            <img src="/images/projects/nrm-thumb.png"  alt="nrm.org.uk" class="frontend overlay"  rel="/projects/nrm"  />
            </a>
        </li>        
        <li class="omega">
            <a href="/projects/snog" title="Snog Frozen Yogurt">
            <img src="/images/projects/snog-thumb.png"  alt="ifancyasnog.com" class="frontend cms wordpress overlay" rel="/projects/snog" />
            </a>
        </li>
        <li class="alpha">
            <a href="/projects/ltm-sense" title="LTM Exhibiton 'Sense in the City'">
            <img src="/images/projects/ltm-sense-thumb.png"  alt="" class="cms php-framework exhibition overlay" rel="/projects/ltm-sense" />
            </a>
        </li>
        <li>
            <a href="/projects/yoobi" title="Yoobi">
            <img src="/images/projects/yoobi-thumb.png"  alt="loveyoobi.com" class="cms frontend wordpress overlay" rel="/projects/yoobi" />
            </a>
        </li> 

        <li>
            <a href="/projects/vinspired" title="Vinspired Voicebox">
            <img src="/images/projects/voicebox-thumb.png"  alt="voicebox.vinspired.com" class="cms frontend php-framework exhibition overlay" rel="/projects/vinspired"   />
            </a>
        </li>
        <li class="omega">
            <a href="/projects/hallagenna" title="Hallagenna Riding Stables">
            <img src="/images/projects/hallagenna-thumb.png"  alt="doolittlesdispensary.com" class="cms frontend joomla overlay" rel="/projects/hallagenna"   />
            </a>
        </li>     
    </ul>
</div>
<div class="jqmWindow" id="project-display">

Please wait... <img src="inc/busy.gif" alt="loading" />
</div>